...
namespace CDR.DataHolder.IntegrationTests
{
    public class US15221_US12969_US15586_MDH_InfosecProfileAPI_Registration_GET : BaseTest, IClassFixture<RegisterSoftwareProductFixture>
    {
        ...
        {
            Guid hiddenSoftwareProductId = Guid.NewGuid();

            void HideSoftwareProduct()
            {
                ...
                using (var connection = new SqlConnection(IDENTITYSERVER_CONNECTIONSTRING))
                {
                    ...
                    if (connection.ExecuteScalarInt32($@"select count(*) from clients where clientid = '{ SOFTWAREPRODUCT_ID.ToLower() }'") != 0)
                    {
                        throw new Exception("Error hiding softwareproduct - idsvr");
                    }
                }
                ...
                using (var connection = new SqlConnection(DATAHOLDER_CONNECTIONSTRING))
                {
                    ...
                    connection.ExecuteNonQuery($@"UPDATE softwareproduct SET softwareproductid = '{hiddenSoftwareProductId}' WHERE softwareproductid = '{ SOFTWAREPRODUCT_ID }'");

                    if (connection.ExecuteScalarInt32($@"select count(*) from softwareproduct where softwareproductid = '{ SOFTWAREPRODUCT_ID }'") != 0)
                    {
                        throw new Exception("Error hiding softwareproduct -  mdh");
                    }
                }
            }

            void RestoreSoftwareProduct()
            {
                ...
                using (var connection = new SqlConnection(IDENTITYSERVER_CONNECTIONSTRING))
                {
                    ...
                    if (connection.ExecuteScalarInt32($@"select count(*) from clients where clientid = '{ SOFTWAREPRODUCT_ID.ToLower() }'") != 1)
                    {
                        throw new Exception("Error restoring softwareproduct - idsvr");
                    }
                }
                ...
                using (var connection = new SqlConnection(DATAHOLDER_CONNECTIONSTRING))
                {
                    ...
                    string sqlQuery = string.Format($"UPDATE SoftwareProduct SET softwareproductid = '{SOFTWAREPRODUCT_ID.ToLower()}' WHERE softwareproductid = '{hiddenSoftwareProductId}'");
                    connection.ExecuteNonQuery(sqlQuery);

                    if (connection.ExecuteScalarInt32($@"select count(*) from softwareproduct where softwareproductid = '{ SOFTWAREPRODUCT_ID }'") != 1)
                    {
                        throw new Exception("Error restoring softwareproduct -  mdh");
                    }
                }
            }
