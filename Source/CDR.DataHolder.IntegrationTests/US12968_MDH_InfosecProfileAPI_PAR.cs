...
namespace CDR.DataHolder.IntegrationTests
{
    public class US12968_MDH_InfosecProfileAPI_PAR : BaseTest, IClassFixture<RegisterSoftwareProductFixture>
    {
        ...
        public async Task AC01_Post_ShouldRespondWith_201Created_RequestUri()
        {
            ...
            // Act
            var response = await DataHolder_Par_API.SendRequest(scope: BaseTest.SCOPE);
        ...
        [InlineData(ADDITIONAL_JWKS_CERTIFICATE_FILENAME, ADDITIONAL_JWKS_CERTIFICATE_PASSWORD, HttpStatusCode.BadRequest)] // ie different holder of key
        ...
        {
            // Arrange
            var codeVerifier = string.Concat(System.Guid.NewGuid().ToString(), '-', System.Guid.NewGuid().ToString());
            var response = await DataHolder_Par_API.SendRequest(codeVerifier: codeVerifier, scope: US12963_MDH_InfosecProfileAPI_Token.SCOPE_TOKEN_OPENID_ACCOUNTS);
            ...
            (var authCode, var idToken, var _) = await new DataHolder_Authorise_APIv2
            {
                UserId = USERID_MARYMOSS,
                OTP = AUTHORISE_OTP,
                SelectedAccountIds = ACCOUNTIDS_ALL_MARY_MOSS,
                Scope = US12963_MDH_InfosecProfileAPI_Token.SCOPE_TOKEN_OPENID_ACCOUNTS,
        ...
        public async Task<(string? cdrArrangementId, string? requestUri, string? codeVerifier)> AC04_Post_WithCdrArrangementId_ShouldRespondWith_201Created_RequestUri()
        {
            // Create a CDR arrangement
            static async Task<string> CreateCDRArrangement(string codeVerifier)
            {
                // Authorise
                (var authCode, var _, var _) = await new DataHolder_Authorise_APIv2
                {
                    UserId = USERID_MARYMOSS,
                    OTP = AUTHORISE_OTP,
                    SelectedAccountIds = ACCOUNTIDS_ALL_MARY_MOSS,
            ...
            // Arrange 
            var codeVerifier = string.Concat(System.Guid.NewGuid().ToString(), '-', System.Guid.NewGuid().ToString());
            var cdrArrangementId = await CreateCDRArrangement(codeVerifier);

            // Act - PAR with existing CdrArrangementId
            var response = await DataHolder_Par_API.SendRequest(codeVerifier: codeVerifier, scope: US12963_MDH_InfosecProfileAPI_Token.SCOPE_TOKEN_OPENID_ACCOUNTS, cdrArrangementId: cdrArrangementId);
            ...
            using (new AssertionScope())
            {
                ...
                return (cdrArrangementId, parResponse?.RequestURI, codeVerifier);
            }
        }
        ...
        public async Task AC05_Post_AuthorisationEndpoint_WithRequestUri_ShouldRespondWith_200OK_CdrArrangementId()
        {
            ...
            (var cdrArrangementId, var requestUri, var codeVerifier) = await AC04_Post_WithCdrArrangementId_ShouldRespondWith_201Created_RequestUri();
