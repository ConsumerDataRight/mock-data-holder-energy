...
namespace CDR.DataHolder.IntegrationTests
{
    ...
    {
        public const string SCOPE_TOKEN_OPENID = "openid";
        public const string SCOPE_TOKEN_OPENID_ACCOUNTS = "openid energy:accounts.basic:read";
        public const string SCOPE_TOKEN_OPENID_ACCOUNTS_ADDITIONAL = "openid energy:accounts.basic:read additional:scope";
        ...
        public async Task AC01_Post_ShouldRespondWith_200OK_IDToken_AccessToken_RefreshToken()
        {
            ...
            {
                UserId = USERID_MARYMOSS,
                OTP = AUTHORISE_OTP,
                SelectedAccountIds = ACCOUNTIDS_ALL_MARY_MOSS,
                Scope = SCOPE_TOKEN_OPENID_ACCOUNTS,
            }.Authorise();

            // Act
            var responseMessage = await DataHolder_Token_API.SendRequest(authCode, codeVerifier: codeVerifier);
            ...
            using (new AssertionScope())
            {
                ...
                if (responseMessage.StatusCode == HttpStatusCode.OK)
                {
                    ...
                    tokenResponse?.Scope.Should().Be(SCOPE_TOKEN_OPENID_ACCOUNTS);
        ...
        public async Task AC02_Put_ShouldRespondWith_404NotFound()
        {
            ...
            // Act
            var responseMessage = await DataHolder_Token_API.SendRequest(authCode, usePut: true, codeVerifier: codeVerifier);
        ...
        public async Task AC03_Post_WithInvalidRequest_GrantType_ShouldRespondWith_400BadRequest_InvalidRequestErrorResponse(string grantType, HttpStatusCode expectedStatusCode)
        {
            ...
            // Act
            var responseMessage = await DataHolder_Token_API.SendRequest(authCode, codeVerifier: codeVerifier, grantType: grantType);
        ...
        public async Task AC03_Post_WithInvalidRequest_ClientId_ShouldRespondWith_400BadRequest_InvalidClientErrorResponse(string clientId, HttpStatusCode expectedStatusCode)
        {
            ...
            // Act
            var responseMessage = await DataHolder_Token_API.SendRequest(authCode, codeVerifier: codeVerifier, clientId: clientId);
        ...
        public async Task AC03_Post_WithInvalidRequest_ClientAssertionType_ShouldRespondWith_400BadRequest_InvalidClientErrorResponse(string clientAssertionType, HttpStatusCode expectedStatusCode)
        {
            ...
            // Act
            var responseMessage = await DataHolder_Token_API.SendRequest(authCode, codeVerifier: codeVerifier, clientAssertionType: clientAssertionType);
        ...
        public async Task AC03_Post_WithInvalidRequest_ClientAssertion_ShouldRespondWith_400BadRequest_InvalidClientErrorResponse(bool useClientAssertion, HttpStatusCode expectedStatusCode)
        {
            ...
            // Act
            var responseMessage = await DataHolder_Token_API.SendRequest(authCode, codeVerifier: codeVerifier, useClientAssertion: useClientAssertion);
        ...
        public async Task AC04_Post_WithInvalidClientAssertion_ShouldRespondWith_400BadRequest_InvalidClientErrorResponse(AC04_TestType testType, HttpStatusCode expectedStatusCode)
        {
            ...
            var responseMessage = await DataHolder_Token_API.SendRequest(authCode, codeVerifier: codeVerifier, customClientAssertion: clientAssertion);
        ...
        public async Task AC05_Post_WithExpiredAuthCode_ShouldRespondWith_400BadRequest_InvalidGrant(bool expired, HttpStatusCode expectedStatusCode)
        {
            static async Task ExpireAuthCode(string authCode)
            {
                ...
                {
                    // Not sure how to derive persisted grant key from authcode, so lets just check only 1 authorization_code row and use it's key // TODO - ideally, derive persisted grant key from authcode
        ...
        public async Task AC08_Post_WithShareDuration_ShouldRespondWith_200OK_IDToken_AccessToken_RefreshToken(int shareDuration)
        {
            ...
            // Act
            var responseMessage = await DataHolder_Token_API.SendRequest(authCode, codeVerifier: codeVerifier, shareDuration: shareDuration);
        ...
        private async Task<HttpResponseMessage> Test_AC09_AC10(string initalScope, string requestedScope)
        {
            static async Task<(string? authCode, string? refreshToken)> GetRefreshToken(string scope)
            {
                ...
                var responseMessage = await DataHolder_Token_API.SendRequest(authCode, codeVerifier: codeVerifier, shareDuration: 3600);
        ...
        {
            // Arrange/Act
            var responseMessage = await Test_AC09_AC10(SCOPE_TOKEN_OPENID_ACCOUNTS, SCOPE_TOKEN_OPENID_ACCOUNTS);
        ...
        [Theory]
        [InlineData(SCOPE_TOKEN_OPENID_ACCOUNTS, SCOPE_TOKEN_OPENID_ACCOUNTS, HttpStatusCode.OK)] // Same scope - should be ok
        [InlineData(SCOPE_TOKEN_OPENID_ACCOUNTS, SCOPE_TOKEN_OPENID, HttpStatusCode.OK)] // Decreased in scope - should be ok
        [InlineData(SCOPE_TOKEN_OPENID, SCOPE_TOKEN_OPENID_ACCOUNTS, HttpStatusCode.OK)] // Increased in scope - should filter and pass
        [InlineData(SCOPE_TOKEN_OPENID_ACCOUNTS, SCOPE_TOKEN_OPENID_ACCOUNTS_ADDITIONAL, HttpStatusCode.OK)] // Exceed the allowable scope of the DR - should filter and pass
        ...
        public async Task AC11_Post_WithInvalidRefreshToken_ShouldRespondWith_400BadRequest_InvalidFieldErrorResponse(string refreshToken, HttpStatusCode expectedStatusCode)
        {
            ...
            // Act
            var responseMessage = await DataHolder_Token_API.SendRequest(authCode, codeVerifier: codeVerifier, grantType: "refresh_token", refreshToken: refreshToken);
        ...
        public async Task ACX02_UseRefreshTokenMultipleTimes_ShouldRespondWith_AccessTokenRefreshToken(int usageAttempts)
        {
            ...
            using (new AssertionScope())
            {
                ...
                for (int i = 1; i <= usageAttempts; i++)
                {
                    ...
                    refreshTokenResponse?.RefreshToken.Should().Be(refreshToken); // the same refresh token is returned 
