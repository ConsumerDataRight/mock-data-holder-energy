...
namespace CDR.GetDataRecipients
{
    public static class GetDataRecipientsFunction
    {
        ...
        public static async Task DATARECIPIENTS([TimerTrigger("%Schedule%")] TimerInfo myTimer, ILogger log, ExecutionContext context)
        {
            try
            {
                ...
                if (respStatusCode == System.Net.HttpStatusCode.OK)
                {
                    ...
                    if (ex != null)
                    {
                        log.LogError(ex, "An exception occurred retrieving data recipients from the database.");
                        throw ex;
                    }

                    // Compare Register and Data Holder -> Data Recipients
                    await CompareDataRecipients(regDataRecipients, dhDataRecipients, log);
                }
            }
            catch (Exception ex)
            {
        ...
        private static async Task CompareDataRecipients(IList<LegalEntity> regDataRecipients, IList<LegalEntity> dhDataRecipients, ILogger log)
        {
            try
            {
